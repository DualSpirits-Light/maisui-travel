...
public final class TripShareArchive {
    ...
    public static void write(Context context,Trip trip,OutputStream target)throws Exception {
        byte[] tripJson=TripStore.encode(Collections.singletonList(trip)).getBytes(StandardCharsets.UTF_8);
        ...
        try(ZipOutputStream zip=new ZipOutputStream(new BufferedOutputStream(new LimitedOutputStream(target,MAX_ARCHIVE)))){
            ...
            JSONObject manifest=new JSONObject().put("format","maisui-trip").put("version",FORMAT_VERSION).put("createdAt",System.currentTimeMillis()).put("title",trip.title).put("files",files).put("media",mediaMap);
    ...
    public static Trip importTrip(Context context,File archive,TripStore store,List<Trip> existing)throws Exception {
        ...
        try{
            Loaded loaded=load(stage.dir);Trip trip=loaded.trip;regenerateIds(trip);
            ...
            ArrayList<Trip> merged=new ArrayList<>(existing);trip.pinned=false;trip.archived=false;trip.favorite=false;trip.updatedAt=System.currentTimeMillis();merged.add(trip);TripStore.sort(merged);
            store.save(merged);existing.clear();existing.addAll(merged);committed=true;return trip;
    ...
    private static final class Loaded {final Trip trip;final JSONObject media;Loaded(Trip trip,JSONObject media){this.trip=trip;this.media=media;}}
    ...
    private static Loaded load(File stage)throws Exception {
        ...
        JSONObject manifest=new JSONObject(read(manifestFile,2*1024*1024));if(!"maisui-trip".equals(manifest.optString("format"))||manifest.optInt("version")!=FORMAT_VERSION)throw new IOException("不支持此分享文件版本");
        ...
        ArrayList<Trip> trips=TripStore.decode(read(tripFile,16*1024*1024));if(trips.size()!=1)throw new IOException("分享文件必须只包含一个旅行");Trip trip=trips.get(0);validateMediaMap(trip,media,stage,expected);return new Loaded(trip,media);
    }
