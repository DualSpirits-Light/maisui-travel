...
public final class AppFileProvider extends ContentProvider {
    ...
    public static Uri uriFor(Context c,File source){try{File f=source.getCanonicalFile(),media=new File(c.getFilesDir(),"media").getCanonicalFile(),camera=new File(c.getCacheDir(),"camera").getCanonicalFile(),updates=new File(c.getCacheDir(),"updates").getCanonicalFile();String kind,relative;if(within(f,media)){kind="media";relative=rel(media,f);}else if(within(f,camera)){kind="camera";relative=rel(camera,f);}else if(within(f,updates)){kind="updates";relative=rel(updates,f);}else throw new IllegalArgumentException("File is outside shared app folders");Uri.Builder b=new Uri.Builder().scheme("content").authority(c.getPackageName()+".files").appendPath(kind);for(String part:relative.split("/"))b.appendPath(part);return b.build();}catch(IOException e){throw new IllegalArgumentException("Invalid file",e);}}
