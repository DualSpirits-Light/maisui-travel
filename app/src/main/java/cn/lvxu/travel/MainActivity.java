...
public class MainActivity extends Activity {
    ...
    void itinerary(){
        ...
        if(mapMode){mapUi=new AmapUi(this);mapUi.show(schedule);mapUi.resume();}else{long cost=0;for(Trip.Stop s:schedule)cost+=s.cost;body.addView(text(schedule.size()+" 个地点 · 预计 ¥"+Trip.money(cost),13,MUTED));space(body,14);if(schedule.isEmpty()){LinearLayout c=card(body);c.addView(bold("今天，想去哪里？",20,INK));space(c,8);c.addView(text("添加地点或粘贴高德分享链接。",14,MUTED));}int lastEnd=-1;
            for(int i=0;i<schedule.size();i++){Trip.Stop s=schedule.get(i);LinearLayout c=card(body);PlaceMediaUi.preview(this,c,s);c.addView(bold(String.format(Locale.ROOT,"%02d   %s · %d 分钟",i+1,s.time,s.duration),15,INK));space(c,12);c.addView(bold(s.name,20,INK));space(c,8);c.addView(text("抵达："+s.mode+" / 预计 ¥"+Trip.money(s.cost),13,MUTED));if(!s.address.isEmpty()){space(c,7);c.addView(text(s.address,13,MUTED));}if(!s.openingHours.isEmpty()){space(c,7);c.addView(text("营业时间："+s.openingHours,13,MUTED));}if(s.rating!=null){space(c,7);c.addView(text("评分："+s.rating+" / 5 · 来源信息请核实",13,MUTED));}if(!s.note.isEmpty()){space(c,9);c.addView(text(s.note,14,MUTED));}PlaceMediaUi.notes(this,c,s);int start=LocalTime.parse(s.time).toSecondOfDay()/60;if(start<lastEnd){space(c,8);c.addView(text("与前面的安排时间重叠，请检查。",12,ORANGE));}lastEnd=Math.max(lastEnd,start+s.duration);space(c,14);pair(c,action("高德查看 ↗",false,()->map(s)),action("编辑地点",false,()->stopEditor(s)));c.setOnLongClickListener(v->{confirm("删除“"+s.name+"”？",()->{active.stops.remove(s);changed();});return true;});}
        }
    ...
    private void editStop(Trip.Stop original,Trip.Stop s){
        ...
        dialog(original==null?"添加地点":"编辑地点",f,()->{String n=required(name,120);int dur=number(duration,1,1440);long c=Trip.cents(cost.getText().toString());String notes=note.getText().toString().trim(),addr=address.getText().toString().trim(),open=hours.getText().toString().trim();if(notes.length()>2000||addr.length()>300||open.length()>300)throw new IllegalArgumentException("文字内容过长");String la=lat.getText().toString().trim(),lo=lon.getText().toString().trim();Double latitude=null,longitude=null;Float score=null;
            ...
            String startTime=time.getText().toString().trim();try{LocalTime.parse(startTime);if(!startTime.matches("\\d{2}:\\d{2}"))throw new IllegalArgumentException();}catch(Exception ex){throw new IllegalArgumentException("请选择有效开始时间，如 09:00");}photos.apply(s);s.name=n;s.time=startTime;s.day=chosen.getSelectedItemPosition();s.duration=dur;s.mode=mode.getSelectedItem().toString();s.cost=c;s.address=addr;s.openingHours=open;s.rating=score;s.lat=latitude;s.lon=longitude;s.coordinateSystem=prefs.showPlaceCoordinates()?coord.getSelectedItem().toString():s.coordinateSystem;s.note=notes;if(original==null)target.stops.add(s);else{int index=target.stops.indexOf(original);if(index<0)throw new IllegalArgumentException("该地点已被删除");target.stops.set(index,s);}active=target;day=s.day;changed();
