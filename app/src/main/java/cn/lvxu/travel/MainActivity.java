...
public class MainActivity extends Activity {
    ...
    private void editStop(Trip.Stop original,Trip.Stop s){
        ...
        dialog(original==null?"添加地点":"编辑地点",f,()->{String n=required(name,120);int dur=number(duration,1,1440);long c=Trip.cents(cost.getText().toString());String notes=note.getText().toString().trim(),addr=address.getText().toString().trim(),open=hours.getText().toString().trim();if(notes.length()>2000||addr.length()>300||open.length()>300)throw new IllegalArgumentException("文字内容过长");String la=lat.getText().toString().trim(),lo=lon.getText().toString().trim();Double latitude=null,longitude=null;Float score=null;
            ...
            photos.apply(s);s.name=n;s.time=time.getText().toString();s.day=chosen.getSelectedItemPosition();s.duration=dur;s.mode=mode.getSelectedItem().toString();s.cost=c;s.address=addr;s.openingHours=open;s.rating=score;s.lat=latitude;s.lon=longitude;s.coordinateSystem=prefs.showPlaceCoordinates()?coord.getSelectedItem().toString():s.coordinateSystem;s.note=notes;if(original==null)target.stops.add(s);else{int index=target.stops.indexOf(original);if(index<0)throw new IllegalArgumentException("该地点已被删除");target.stops.set(index,s);}active=target;day=s.day;changed();
