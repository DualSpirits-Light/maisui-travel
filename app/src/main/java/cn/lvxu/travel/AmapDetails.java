...
final class AmapDetails {
    static void enrich(android.content.Context a,PlaceImporter.Place p)throws Exception{
        ...
        else if(!p.name.isEmpty()) {PoiResultV2 result=search.searchPOI();if(result!=null&&result.getPois()!=null)for(PoiItemV2 item:result.getPois()){if(normal(item.getTitle()).equals(normal(p.name))&&!p.address.isEmpty()&&normal(item.getSnippet()).contains(normal(p.address))){if(match!=null)return;match=item;}}}
        if(match!=null)fill(p,match);
    }
