...
public final class MediaController {
    ...
    void attachCheckin(CheckinUi ui){
        ...
        if(checkinCapture)callback=path->ui.photoSelected(path,checkinPhotoKind);
    }
    ...
    public void saveState(Bundle b){if(cameraFile!=null)b.putString("media.camera",cameraFile.getAbsolutePath());b.putBoolean("media.checkin",checkinCapture);b.putString("media.checkin.kind",checkinPhotoKind);if(checkin!=null)checkin.saveState(b);}
    public void restoreState(Bundle b){String p=b.getString("media.camera");if(p!=null)try{File candidate=new File(p).getCanonicalFile(),root=new File(a.getCacheDir(),"camera").getCanonicalFile();if(candidate.getPath().startsWith(root.getPath()+File.separator))cameraFile=candidate;}catch(IOException ignored){}checkinCapture=b.getBoolean("media.checkin");checkinPhotoKind=b.getString("media.checkin.kind","group");restoredState=new Bundle(b);}
